...
export default function AgentSignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      // Create auth user; attach metadata so we can build profile
      ...
      // If user exists (email confirmations off), insert profile immediately
      ...
      if (user) {
        const { error: upsertError } = await supabase
          ...
          .insert({
            id: user.id,
            email: user.email,
            full_name: formData.fullName || user.email || '',
        ...
        // If email confirmation required, send them to login
    ...
    <div className="min-h-screen flex items-center justify-center bg-kasi-black p-4">
      <div className="bg-gray-900 p-8 rounded-lg shadow-2xl w-full max-w-md border border-gray-800">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 mt-1">Agent Portal</p>
        </div>
        ...
        <div className="mt-6 text-center">
          <Link href="/agent/login" className="text-kasi-blue hover:text-opacity-80 text-sm">

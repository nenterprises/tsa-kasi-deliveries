          ...
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="min-w-0 hidden sm:block">
                <p className="text-xs text-gray-400">Store Portal</p>
                <p className="font-semibold text-white text-sm truncate" title={storeSession?.storeName}>{storeSession?.storeName}</p>
              </div>
            </div>

            <button
              onClick={handleLogout}
              className="px-4 py-2 text-sm text-red-400 hover:bg-red-900/20 rounded-lg transition-all border border-red-900/30 hover:border-red-800 flex items-center gap-2 flex-shrink-0"
            >
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`px-4 py-3 text-sm font-medium whitespace-nowrap rounded-lg transition-all ${
                    isActive
